...
- The fix was to move away from ssh_channel_request_exec() and use
   ...
   * ssh_channel_write(channel, line, strlen(line)) - send the actual commands
- While technically you could run the script without the pty portion of the code that would make it non-interactive shell. I didn't like that so I included it making it interactive-shell.
